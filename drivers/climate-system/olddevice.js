  ...
  async onInit() {
    ...
    console.log('### Run once; and later run at interval defined in user settings');
            ...
            cookie = preauthResponse.headers.get('Set-Cookie');
                ...
                //Check on newer APP version if Store contains SystemID and GateWayID
                const deviceId = this.getStoreValue("deviceId");

                if (deviceId) {
                    console.log("Device Store for SystemID exists:", deviceId);
                    vargatewayId = this.getStoreValue("deviceGateway");
                    varsystemId = this.getStoreValue("deviceId");
                } else {
                    console.log("Device Store for SystemID is empty or undefined.");
    ...
    console.log('### Here we go again...');
              ...
              cookie = authResponse.headers.get('Set-Cookie');
                  ...
                  console.log('System ID:', varsystemId); // Echo the system_id to the console log
                          ...
                          const pagesArray = details.menuItems[0].pages;
                          console.log("Full Pages Array:", JSON.stringify(pagesArray, null, 2));
                          ...
                          // Status BYPASS:
                          const homePage2 = pages[1] || {};
                          const parameters2 = homePage2.parameterDescriptors || [];
                          const bypass = parameters2.find(param => param.name === 'Status Bypassklappe');
                          let bypass_value = bypass.value
                          let bypass_status = 'N/A'; // Use 'let' since the value changes
                          ...
                          // Assign based on `bypass.value`, defaulting to 'N/A' if not found
                          bypass_status = statusMapping[bypass.value] || "N/A";

                          //this.setCapabilityValue('operational_state.bypass', bypass_value);
                          
                          console.log('%%%%%%%%% BYPASS Status: ' + bypass_status + ' , ' + bypass.value);
                          //console.log('%%%%%%%%% BYPASS valueId: ' + bypass_valueId);
    ...
    console.log('#DeviceId: ' + this.getStoreValue("deviceId"));
